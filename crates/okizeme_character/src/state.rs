...
//   pub fn update(&mut self, buffer: &mut Buffer, movement: &mut Movement, velocity: &mut Velocity, name: &Name, position: Vec3) -> Option<AnimationTransition> {
//     use CharacterState::*;
//     self.tick();
    
//     let new_state = match self {
//       Idle | Walking | BackWalking | Crouching => self.from_neutral_states(buffer, movement, velocity, name),
//       Dashing => self.from_dashing(buffer, movement),
//       Jumpsquat { duration:_,velocity:_ } => self.from_jump_squat(movement, velocity),
//       Rising { busy: _ } | Falling => self.from_neutral_airborne(buffer, movement, velocity, name, position),
//       BackDashing { duration:_ } => self.from_backdashing(buffer, movement, velocity, name),
//       Attacking {duration:_, attack:_, cancellable:_} => self.from_attacking(buffer, movement, velocity, name),
//       AirDashing { busy:_,duration:_,velocity:_} | AirBackDashing { busy:_,duration:_,velocity:_} => self.from_air_dashing(buffer, movement, velocity, name),
//       _ => self.clone()
//     };
...
//   ///  - Backwalking
//   ///  - Crouching
//   pub fn from_neutral_states(&self, buffer: &Buffer, movement: &mut Movement, velocity: &mut Velocity, name: &Name)  -> Self {
//     use CharacterState::*;
...
//   pub fn from_jump_squat(&self, movement: &mut Movement, velocity: &mut Velocity) -> Self{
//     use CharacterState::*;
//     match self {
...
//   pub fn from_air_dashing(&self, buffer: &Buffer, movement: &mut Movement, velocity: &mut Velocity, name: &Name) -> Self {
//     use CharacterState::*;
//     match self {
//       AirDashing {busy:_ ,duration, velocity:_} => {
//         if *duration == 0 {
//           return self.from_neutral_airborne(buffer, movement, velocity, name, Vec3::ONE);
//         }
//         return self.clone();
//       },
...
//   ///  - Airdashing
//   ///  - Airbackdashing
//   pub fn from_neutral_airborne(&self, buffer: &Buffer, movement: &mut Movement, velocity: &mut Velocity, name: &Name, position: Vec3) -> Self {
//     use CharacterState::*;
//     if position.y <= 0.0 {
//       return Idle;
//     }
...
//       Rising { busy } => {
//         if *busy == 0 {
//           return self.from_airborne_input(buffer, movement, velocity, name);
//         } else {
//           return self.clone();
//         }
//       },
...
//   pub fn from_backdashing(&self, buffer: &Buffer, movement: &mut Movement, velocity: &mut Velocity, name: &Name) -> Self {
//     use CharacterState::*;
//     match self {
//       BackDashing {duration} => {
//         if *duration == 0 {
//           return self.from_neutral_states(buffer, movement, velocity, name);
//         }
//         return self.clone();
//       },
//       _ => return self.clone(),
...
//   pub fn from_attacking(&self, buffer: &Buffer, movement: &mut Movement, velocity: &mut Velocity, name: &Name) -> Self {
//     use CharacterState::*;
//     match self {
...
//   pub fn from_airborne_input(&self, buffer: &Buffer, movement: &mut Movement, velocity: &mut Velocity, name: &Name) -> Self {
//     use CharacterState::*;
...
impl CharacterState {
  ...
  pub fn update(&mut self, buffer: &mut Buffer, movement: &mut Movement, velocity: &mut Velocity, name: &Name, position: Vec3) -> Option<AnimationTransition> {
    ...
    let new_state = match self {
      ...
      Dashing => self.from_dashing(buffer, movement),
      Jumpsquat { duration:_,velocity:_ } => self.from_jump_squat(movement, velocity),
      Rising | Falling => self.from_neutral_airborne(buffer, movement, velocity, name, position),
      BackDashing { duration:_ } => self.from_backdashing(buffer, movement, velocity, name),
      Attacking {duration:_, attack:_, cancellable:_} => self.from_attacking(buffer, movement, velocity, name),
      AirDashing {duration:_,velocity:_} | AirBackDashing {duration:_,velocity:_} => self.from_air_dashing(buffer, movement, velocity, name),
      _ => self.clone()
    };
    let transition = if self.clone() != new_state {
      self.calculate_transition(&new_state)
