...
pub fn manage_character_state(
    ...
    mut query: Query<(&PlayerId, &Name, &mut CharacterState, &mut Movement, &mut Velocity), (Without<Hitstop>, Without<Busy>)>,
    mut transition_writer: EventWriter<AnimationTransitionEvent>,
  ) {
    for (player_id, name, mut state, mut movement, mut velocity) in query.iter_mut() {
      let position = player_data.get_position(player_id);
      for buffer in player_data.buffers.iter_mut() {
        if buffer.player_id == *player_id {
          let transition = state.update(buffer,&mut movement, &mut velocity, name, position);
