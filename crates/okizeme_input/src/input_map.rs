...
impl InputMap {
    pub fn get_raw_input_frame(&self, keyboard_input: &Res<Input<KeyCode>>, button_input: &Res<Input<GamepadButton>>) -> RawInputFrame {
      ...
      let a_released = match self.a {
        ...
        RawButton::G(device_id,button_type) => button_input.just_released(GamepadButton(device_id, button_type)),
      };
      ...
      let h_released = match self.h {
        ...
        if a_pressed {pressed_byte |= 0b0000_0001}
        if b_pressed {pressed_byte |= 0b0000_0010}
        if c_pressed {pressed_byte |= 0b0000_0100}
        if d_pressed {pressed_byte |= 0b0000_1000}
        if e_pressed {pressed_byte |= 0b0001_0000}
        if f_pressed {pressed_byte |= 0b0010_0000}
        if g_pressed {pressed_byte |= 0b0100_0000}
        if h_pressed {pressed_byte |= 0b1000_0000}

        let mut released_byte: u8 = 0b0000_0000;
        if a_released {released_byte |= 0b0000_0001}
        if b_released {released_byte |= 0b0000_0010}
        if c_released {released_byte |= 0b0000_0100}
        if d_released {released_byte |= 0b0000_1000}
        if e_released {released_byte |= 0b0001_0000}
        if f_released {released_byte |= 0b0010_0000}
        if g_released {released_byte |= 0b0100_0000}
        if h_released {released_byte |= 0b1000_0000}

        let mut held_byte: u8 = 0b0000_0000;
        if a_held {held_byte |= 0b0000_0001}
        if b_held {held_byte |= 0b0000_0010}
        if c_held {held_byte |= 0b0000_0100}
        if d_held {held_byte |= 0b0000_1000}
        if e_held {held_byte |= 0b0001_0000}
        if f_held {held_byte |= 0b0010_0000}
        if g_held {held_byte |= 0b0100_0000}
        if h_held {held_byte |= 0b1000_0000}
