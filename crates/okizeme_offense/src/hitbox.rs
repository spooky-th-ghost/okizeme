...
pub trait SpawnHitbox {
     fn spawn_hitbox(&mut self, player_id: &PlayerId, attack_event: &AttackEvent, parent_transform: &Transform, facing_vector: f32, is_visible: bool);
}

impl SpawnHitbox for Commands<'_, '_>{
  fn spawn_hitbox(&mut self, player_id: &PlayerId, attack_event: &AttackEvent, parent_transform: &Transform, facing_vector: f32, is_visible: bool) {
    let offset = Vec3::new(attack_event.location.x * facing_vector, attack_event.location.y, 1.0);
